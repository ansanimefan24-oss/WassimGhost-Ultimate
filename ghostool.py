...
class WassimGhostUltimate:
    ...
    def nmap_module(self):
        self.clear()
        console.print(Panel("[bold cyan]WASSIM ULTRA SCANNER (ALL PORTS 1 - 65535)[/bold cyan]"))
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=500) as executor:
            ports = range(1, 65536)
            results = list(track(executor.map(lambda p: self.scan_single_port(target, p), ports), total=len(ports), description="Scanning All Ports..."))
        for p in results:
            if p:
                open_ports.append(p)
        ...
        if not open_ports:
            ...
            console.print(table)
            console.print(f"[bold green][+] Total Open Ports: {len(open_ports)}[/bold green]")
    ...
    def main_loop(self):
        while True:
            ...
            menu_table.add_row("[bold cyan]1.[/bold cyan] SCAN ALL PORTS (1 - 65535)")
